...
* The `>` prompt on BIOS-based AMD64 and Intel{nbsp}64 systems, accessed by highlighting an entry in the boot menu and pressing the kbd:[Tab] key. Unlike the `boot:` prompt, this prompt allows you to edit a predefined set of boot options. For example, if you highlight the entry labeled `Test this media & install {PRODUCT}{nbsp}{PRODVER}`, a full set of options used by this menu entry will be displayed on the prompt, allowing you to add your own options.
+
...
+
** an ISO image of the full {PRODUCT} installation DVD, placed on a hard drive or a network location accessible from the installation system (requires specifying _NFS Server_ as the installation source)
+
...
In previous releases of {PRODUCT}, there were separate options for an installable tree accessible by NFS (the [option]`nfs` option) and an ISO image located on an NFS source (the [option]`nfsiso` option). In {PRODUCT}{nbsp}{PRODVER}, the installation program can automatically detect whether the source is an installable tree or a directory containing an ISO image, and the [option]`nfsiso` option is deprecated.
...
|===
+
Additionally, the installer will attempt to load a Kickstart file named `ks.cfg` from a volume with a label of `OEMDRV` if present. If your Kickstart file is in this location, you do not need to use the [option]`inst.ks=` boot option at all.
...
+
The _netmask_ parameter is the netmask to be used. This can either be a full netmask for IPv4 (for example `255.255.255.0`) or a prefix for IPv6 (for example `64`).
+
...
|[option]`ksdevice=bootif`|Ignored (the BOOTIF= option is used by default when specified)
...
[option]`upgradeany`::
+
Upgrades are done in a different way in {PRODUCT}{nbsp}{PRODVER}.
...
To test the checksum integrity of an ISO image, append the [option]`rd.live.check` to the boot loader command line. Note that this option is used automatically if you select the default installation option from the boot menu (`Test this media & install {PRODUCT}{nbsp}{PRODVER}`).
