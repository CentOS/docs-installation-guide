...
When you click the `Reconfigure NVDIMM` button, a dialog appears for you to reconfigure the selected NVDIMM devices into sector mode. The reconfiguration lets you use this NVDIMM device as an installation destination.

.Reconfigure NVDIMM

image::{imagesdir}/diskpartadvanced/nvdimm-reconfigure.png[Reconfigure NVDIMM]
...
Select the sector size and press the btn:[Start reconfiguration] button. The supported sector sizes are 512 and 4096 bytes.

Once the device is configured, press btn:[OK] to return to the Installation Destination screen.

.Successfully reconfigured NVDIMM

image::{imagesdir}/diskpartadvanced/nvdimm-reconfigure-success.png[Successfully reconfigured NVDIMM]

NVDIMM devices in sector mode will be displayed under the `NVDIMM Devices` tab in the Installation Destination screen and can now be selected for installation.
