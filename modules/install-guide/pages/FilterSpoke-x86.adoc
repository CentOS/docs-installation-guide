...
* btn:[Reconfigure NVDIMM] - use to reconfigure a NVDIMM device into sector mode; continue with xref:FilterSpoke-x86.adoc#sect-advanced-storage-nvdimm-x86[Reconfigure NVDIMM (x86)]

* btn:[Refresh List] - use to reload the list when a device is added after the installer starts.
...
To use an advanced storage device, you can configure a SCSI over TCP/IP (_iSCSI_) target, Fibre Channel over Ethernet (_FCoE_) Storage Area Network (_SAN_), or Non-Volatile Dual In-line Memory Module (_NVDIMM_) device by clicking the appropriate button in the lower right corner of the Installation Destination screen. For an introduction to iSCSI, see xref:iSCSI.adoc#appe-iscsi-disks[iSCSI Disks].
...
[[sect-advanced-storage-nvdimm-x86]]
=== Configuring NVDIMM Devices

Click the btn:[Reconfigure NVDIMM] button to reconfigure the selected NVDIMM device into sector mode and use it as an installation destination. The btn:[Sector size] drop-down list contains the supported sector sizes of `512` and `4096`.

From the btn:[Sector size] drop-down list, select the sector size and click the btn:[Start Reconfiguration] button.

.Reconfigure NVDIMM

image::{imagesdir}/diskpartadvanced/nvdimm-reconfigure2.png[Reconfigure NVDIMM]
...
When the device is configured, press btn:[OK] to return to the Installation Destination screen.

.Successfully reconfigured NVDIMM

image::{imagesdir}/diskpartadvanced/nvdimm-reconfigure-success2.png[Successfully reconfigured NVDIMM]

NVDIMM devices in sector mode are displayed under the btn:[NVDIMM Devices] tab in the btn:[Installation Destination] window and are available for installation.
