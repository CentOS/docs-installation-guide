...
In previous versions of {PRODUCT}, Kickstart allowed for upgrading the system as well. In {PRODUCT}{nbsp}7.x, this functionality has been removed.
...
====
+
** [option]`--boot-drive=` - Specifies which drive the boot loader should be written to, and therefore which drive the computer will boot from. If you use a multipath device as the boot drive, specify the device using its `disk/by-id/dm-uuid-mpath-WWID` name.
+
** [option]`--leavebootorder` - The installer will add {PRODUCT}{nbsp}{PRODVER} to the top of the list of installed systems in the boot loader, and preserve all existing entries as well as their order.
+
...
+
** [option]`--remove-service` - Do not allow services through the firewall.
+
...
[literal,subs="+quotes,verbatim,macros"]
....
[command]`ignoredisk --only-use=/dev/disk/by-id/dm-uuid-mpath-2416CD96995134CA5D787F00A5AA11017`
[command]`bootloader --location=mbr`
....
+
...
[NOTE]
====
For more information about allocating space when writing to an image file, see section **Host Storage** describing **Image files** in the link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/virtualization_getting_started_guide/sec-virtualization_getting_started-products-storage[Red&nbsp;Hat Enterprise&nbsp;Linux&nbsp;7 Storage Administration Guide].
====
+
...
[command]`nvdimm` (optional)::
+
Performs an action on Non-Volatile Dual In-line Memory Module (NVDIMM) devices. This command uses the following format:
+
[literal,subs="+quotes,verbatim,macros"]
....
[command]`nvdimm _action_ _[options]_pass:attributes[{blank}]`
....
+
[NOTE]
====

By default, all NVDIMM devices are ignored by the installer. You must use the [command]`nvdimm` command to enable installation on these devices.
...
[literal,subs="+quotes,verbatim,macros"]
....
[command]`nvdimm reconfigure [--namespace=pass:attributes[{blank}]_NAMESPACE_pass:attributes[{blank}]] [--mode=pass:attributes[{blank}]_MODE_pass:attributes[{blank}]] [--sectorsize=pass:attributes[{blank}]_SECTORSIZE_pass:attributes[{blank}]]`
....
+
...
[literal,subs="+quotes,verbatim,macros"]
....
[command]`nvdimm use [--namespace=pass:attributes[{blank}]_NAMESPACE_|--blockdevs=pass:attributes[{blank}]_DEVICES_pass:attributes[{blank}]]`
....
+
...
====
+
** [option]`--grow` - Tells the partition to grow to fill available space (if any), or up to the maximum size setting, if one is specified.
+
[NOTE]
====
...
+
** [option]`--onpart=` or [option]`--usepart=` -  Uses existing blank device and format it to the new specified type. For example:
+
[literal,subs="+quotes,verbatim,macros"]
....
...
+
** [option]`--ondisk=` or [option]`--ondrive=` - Creates a partition (specified by the part command) on an existing disk. This command always creates a partition. For example, [option]`--ondisk=sdb` puts the partition on the second SCSI disk on the system.
+
...
The default {PRODUCT}{nbsp}{PRODVER} [application]*mdadm* metadata version is not supported for the boot device.
...
+
** [option]`--ssh` - If this is option is present, then the _password_ string is interpreted as an ssh key value.
+
[IMPORTANT]
====
...
[literal,subs="+quotes,verbatim,macros"]
....
[command]`sshpw --username=root example_password --lock`
....
...
+
The [command]`zerombr` initializes any invalid partition tables that are found on disks and destroys all of the contents of disks with invalid partition tables.  This command is required when performing an installation on an IBM Z system with unformatted Direct Access Storage Device (DASD) disks, otherwise the unformatted  disks are not formatted and used during the installation.
...
You can specify packages by _environment_, _group_, or by their package names. Several environments and groups that contain related packages are defined. See the `repodata/*-comps-_variant_._architecture_.xml` file on the {PRODUCT}{nbsp}{PRODVER} Installation DVD for a list of environments and groups.
...
....
+
This command installs all packages which are part of the `Infrastracture Server` environment. All available environments are described in the `repodata/*-comps-_variant_._architecture_.xml` file on the {PRODUCT}{nbsp}{PRODVER} Installation DVD. Only a single environment can be specified in the Kickstart file.
...
+
Normally, on an AMD64 and Intel 64 system, only packages for this aNormally, on an AMD64 and Intel 64 system, you can install only the `x86_64` and the `noarch` packages. However, with the `--multilib` option, you can automatically install the 32-bit AMD and the i686 Intel system packages available, if any.
+
...
[option]`--nocore`::
+
Disables installation of the `@Core` package group which is otherwise always installed by default. Disabling the `@Core` package group with [option]`--nocore` should be only used for creating lightweight containers; installing a desktop or server system will result in an unusable system.
+
[NOTE]
====
...
Kickstart installations can contain custom scripts which are run when the installer encounters a fatal error - for example, an error in a package that has been requested for installation, failure to start VNC when specified, or an error when scanning storage devices. Installation cannot continue after such an error has occured. The installer will run all [command]`%onerror` scripts in the order they are provided in the Kickstart file. In addition, [command]`%onerror` scripts will be run in the event of a traceback.
...
Kickstart installations support add-ons. These add-ons can expand the basic Kickstart (and Anaconda) functionality in many ways.
